...
from services import schedule_tasks  # your existing scheduler
...
def schedule_assignment(a):
    ...
    a["tasks"] = sorted(
        a["tasks"],
        key=lambda t: (t["start_date"], -int(t.get("importance", 0)), int(t.get("order", 0)))
    )
...
@app.get("/")
def index():
    schedule_all(ASSIGNMENTS)
    return render_template(
        "index.html",
        assignments=ASSIGNMENTS,
        tasks=tasks_by_start(ASSIGNMENTS),
    )
